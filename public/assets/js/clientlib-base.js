	...
	$(window).bind('scroll', function () {
		if (pageUrl !== 'home') {
			if ($(window).scrollTop() > num) {
                ...
                var image = $('#logo').attr('data-main-src');
                $('#logo').attr("src", image);
...
//   var urlParams = new URLSearchParams(window.location.search);
    ...
    if(queryPart!=null)
    {
        queryURL="/bin/infosys/press-release?currentItemsCount="+itemsCount+"&ispr="+ispr+"&isevents="+isevents+queryPart+"&industrychange=False"+"&servicechange=False";
    }
    else
    {
        queryURL="/bin/infosys/press-release?currentItemsCount="+itemsCount+"&ispr="+ispr+"&isevents="+isevents+"&industrychange=False"+"&servicechange=False";
    }   
    ...
    for(i=0;i<pageObjs.length;i++)
    {
        ...
        if(pressReleaseItem && pressReleaseItem.length>0){
           ...
           var articleimage=$(pressReleaseItem).find('.img-responsive');
            var imageclass=$(pressReleaseItem).find('.col-sm-push-10');
           ...
           // console.log("imageclass"+imageclass);
            ...
            if(loadMoreAricleImage =="null" && watermarkURL!=null){
        ...
        if(pressReleaseItem && pressReleaseItem.length>0){
         ...
         //   console.log("outside loop AricleImage"+pageObjs[i].hasOwnProperty('pageWaterMarkImageURL'));
           // console.log("out side loop watermark"+watermarkURL);
            ...
            var imageclass=$(template).find('.col-sm-push-10');
...
$("#articleYearFilter").on("change", function() {
	...
	$.ajax({
        type:'get',
		contentType: "application/json; charset=utf-8",
        url: appendUrlFilterIndex+"&industrychange=False"+"&servicechange=False",    
        processData: true, 
        success: function (responseData) {
            resData = responseData;
...
$("#industriesFilter").on("change", function() {
	...
	$.ajax({
        type:'get',
		contentType: "application/json; charset=utf-8",
        url: appendUrlFilterIndex+"&industrychange=True"+"&servicechange=False",
        processData: true, 
        success: function (responseData) {
            resData = responseData;
            insertHTML(resData); //insert the child elements on load more
...
$("#servicesFilter").on("change", function() {
	...
	$.ajax({
        type:'get',
		contentType: "application/json; charset=utf-8",
        url: appendUrlFilterIndex+"&servicechange=True"+"&industrychange=False",
        processData: true, 
        success: function (responseData) {
            resData = responseData;
            insertHTML(resData); //insert the child elements on load more
